...
// ============================================================
  // CDC LOGIC: Fast Pulse (trx_clk) -> Slow Pulse (clk)
  // ============================================================
  
  logic toggle_fast;
  logic [2:0] sync_stages;
  logic rx_done_slow; // זה הפולס הבטוח שנשתמש בו ב-FSM


  always_ff @(posedge trx_clk or negedge trx_rst_n) begin
      if (!trx_rst_n) 
          toggle_fast <= 1'b0;
      else if (rx_mac_done) 
  ...
  always_ff @(posedge clk or negedge rst_n) begin
      if (!rst_n) begin
          ...
          sync_stages <= {sync_stages[1:0], toggle_fast};
          
          rx_done_slow <= (sync_stages[2] ^ sync_stages[1]);
      end
  end
...
blue <= is_rgb_cmd ? calc_val_shifted(
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    ...
    else if (rx_done_slow && is_rgb_cmd)  // Increment only on pulse
      msg_color_cnt <= msg_color_cnt + 1;
  end

  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) msg_led_cnt <= '0;
    else if (rx_done_slow && is_l_cmd)  // Increment only on pulse
