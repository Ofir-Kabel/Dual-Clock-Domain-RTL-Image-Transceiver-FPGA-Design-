...
BAUD_RATES = ["9600", "57600", "115200", "921600", "1000000"]

IMG_WIDTH = 256
IMG_HEIGHT = 256
IMG_SCALE = 1
IMG_UPDATE_BATCH = 100 # עדכון כל 100 פיקסלים כדי לראות את זה "רץ"
...
class FpgaStudio:
    def __init__(self, root):
        self.root = root
        self.root.title("FPGA Studio v38 - Compact & Clean")
        ...
        self.setup_ui()
        self.load_rgf_data() # טעינת קבצי ה-XLSX/CSV
    ...
    def setup_ui(self):
        ...
        self.cb_baud = ttk.Combobox(top, values=BAUD_RATES, width=8); self.cb_baud.pack(side="left", padx=5); self.cb_baud.set(DEFAULT_BAUD)
        ...
        ttk.Label(top, text="IMG Config:").pack(side="left")
        self.ent_w = ttk.Entry(top, width=4); self.ent_w.pack(side="left"); self.ent_w.insert(0, str(IMG_WIDTH))
        ttk.Label(top, text="x").pack(side="left")
        self.ent_h = ttk.Entry(top, width=4); self.ent_h.pack(side="left"); self.ent_h.insert(0, str(IMG_HEIGHT))
        ttk.Label(top, text="Scale:").pack(side="left", padx=(5,0))
        self.ent_scale = ttk.Entry(top, width=3); self.ent_scale.pack(side="left"); self.ent_scale.insert(0, str(IMG_SCALE))
        ...
        f_spec = ttk.Labelframe(f_right, text="Image Controls", padding=5); f_spec.pack(fill="x")
        b_ready = ttk.Button(f_spec, text="1. Send IMG_READY (Bit 20)", style="Action.TButton", command=self.cmd_img_ready)
        b_ready.pack(side="left", fill="x", expand=True, padx=2); ToolTip(b_ready, "Writes 1 to IMG_STATUS[20] (0x050000)")
        b_start = ttk.Button(f_spec, text="2. Start IMG_READ (Bit 0)", style="Action.TButton", command=self.cmd_img_read)
        b_start.pack(side="left", fill="x", expand=True, padx=2); ToolTip(b_start, "Writes 1 to IMG_CTRL[0] (0x050008)")
    ...
    def build_fields_ui(self):
        ...
        for f in info['fields']:
            ...
            # --- חסימת שדות Reserved (תיקון מבוקש) ---
    ...
    def send_packet(self, cmd_char, addr, data):
            ...
            # --- שלב 1: בניית החבילה (הלוגיקה החדשה שעובדת) ---
            if cmd_char == 'R':
                # שליחת 6 בייטים בלבד לקריאה (כדי שה-FPGA יגיב)
                pkt = bytearray([
                    0x7B,               # {
                    ord('R'),           # R
                    (addr>>16)&0xFF,    # A2
                ...
                # בקריאה ה-Data לא רלוונטי לשליחה, אבל נשתמש ב-0 לתצוגה
                log_data = 0 
            else:
                # שליחת 16 בייטים לכתיבה (כרגיל)
                pkt = bytearray([
                    0x7B, ord(cmd_char), 
            ...
            # --- שלב 2: התצוגה (חזרה לפורמט המקורי והנקי) ---
            # הפורמט: TX (R) >> Addr: 050000 Data: 00000000
    ...
    def cmd_img_read(self):
        try:
            ...
            self.curr_img = Image.new("RGB", (w, h), "black")
            if self.img_window is None or not self.img_window.winfo_exists():
                self.img_window = Toplevel(self.root); self.img_window.title(f"Image Stream {w}x{h}")
        ...
        except: messagebox.showerror("Error", "Check Width/Height")

    # --- Serial & Image Loop ---
    def toggle_conn(self):
        if self.ser:
            self.is_listening = False; self.ser.close(); self.ser = None
            self.btn_conn.config(text="CONNECT", style="Connect.TButton"); self.log("Disconnected", "err")
        else:
            try:
                self.ser = serial.Serial(self.ent_port.get(), int(self.cb_baud.get()), timeout=0.1)
                self.btn_conn.config(text="DISCONNECT", style="Disconnect.TButton")
                self.log("Connected", "rx"); self.start_listening()
    ...
    def process_image(self, data):
        ...
        # עדכון בלייב לפי המנה שהוגדרה (IMG_UPDATE_BATCH)
    ...
    def refresh_image_window(self):
        if self.img_window and self.img_window.winfo_exists() and self.curr_img:
            ...
            self.root.update_idletasks() # הכרחת הממשק לצייר בלייב
